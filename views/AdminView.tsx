...
export const AdminView: React.FC = () => {
    ...
    const handleAddModule = async () => {
        console.log("Botão clicado: Adicionando módulo para o curso", editingCourse.id);

        // Verificação de Integridade
        if (!editingCourse.id) {
            console.error("AdminView Error: Tentativa de adicionar módulo sem course_id");
            alert("Erro: Salve o curso primeiro antes de adicionar módulos.");
            return;
        }

        const title = prompt("Nome do novo módulo:");
        if (!title) {
            console.log("AdminView: Criação de módulo cancelada pelo usuário (sem título).");
            return;
        }
    ...
    const handleDeleteModule = async (moduleId: string) => {
        console.log("Botão clicado: Removendo módulo", moduleId);
    ...
    const handleDeleteLesson = async (lessonId: string) => {
        console.log("Botão clicado: Removendo aula", lessonId);
