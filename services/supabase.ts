...
export const getPosts = async (currentUserId?: string): Promise<Post[]> => {
  if (!supabase) {
    ...
    const mockPosts: Post[] = [
      {
        id: '1', user_id: 'mock1', content: 'Acabei de finalizar o módulo de Tráfego Pago! As estratégias de ROI mudaram meu jogo. Recomendo muito a aula 3.',
        created_at: new Date(Date.now() - 3600000).toISOString(), likes_count: 12, comments_count: 2, user_has_liked: false,
        profile: { id: 'mock1', username: 'pedro.sales', avatar_url: 'https://i.pravatar.cc/150?u=1', role: 'user', email: 'pedro@mail.com' },
        is_saved: false, recent_liked_avatars: ['https://i.pravatar.cc/150?u=2', 'https://i.pravatar.cc/150?u=3', 'https://i.pravatar.cc/150?u=4']
      },
      {
        id: '2', user_id: 'admin1', content: 'Pessoal, nova aula liberada sobre Contabilidade Digital. Fiquem atentos às atualizações do MEI!',
        created_at: new Date(Date.now() - 86400000).toISOString(), likes_count: 45, comments_count: 10, user_has_liked: true,
        profile: { id: 'admin1', username: 'Admin Community', avatar_url: '', role: 'admin', email: 'admin@ecom.com' },
        is_saved: true, recent_liked_avatars: ['https://i.pravatar.cc/150?u=5', 'https://i.pravatar.cc/150?u=6']
      }
    ];
    return mockPosts;
  }

  // Busca robusta com joins
  const { data, error } = await supabase
    .from('posts')
    .select(`
      *,
      profiles:user_id ( id, username, avatar_url, role, full_name ),
      post_likes ( user_id, profiles ( avatar_url ) ),
      saved_posts ( user_id ),
      comments ( count )
    `)
    .order('created_at', { ascending: false });
  ...
  console.log('Posts fetched from DB:', data);

  // Mapeamento seguro com robustez para null/undefined
  const posts: Post[] = (data || []).map((post: any) => {
    // Processa likes para encontrar avatar stack e like status
    const postLikes = post.post_likes || [];
    const userHasLiked = currentUserId ? postLikes.some((l: any) => l.user_id === currentUserId) : false;

    // Extrai avatares dos últimos 3 likes para a stack
    const recentLikedAvatars = postLikes
      .slice(0, 3)
      .map((l: any) => l.profiles?.avatar_url)
      .filter((url: string | null | undefined) => !!url); // Remove nulos
    ...
    // Garante contagem correta de comentários se vier como array ou count object
    // O select 'comments ( count )' retorna [{count: N}] ou array dependendo da query exata, 
    // mas aqui ajustamos para pegar o length se for o caso ou o count.
    // Ajuste da query para trazer apenas count seria user 'comments(count)', mas o supabase-js retorna array de objs.
    // Vamos assumir que 'comments' vem como array para simplificar a contagem se não for agregado.
    // Se a query acima retornar 'comments' como array de objetos { count: ... }, pegamos o primeiro.
    // Mas para segurança, se 'comments' for array normal de itens, usamos length.
    let commentsCount = 0;
    if (Array.isArray(post.comments)) {
      if (post.comments.length > 0 && post.comments[0].count !== undefined) {
    ...
    return {
      ...
      profile: post.profiles || { username: 'Usuário Desconhecido' }, // Fallback para evitar crash
      is_saved: isSaved,
