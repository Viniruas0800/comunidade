...
export const getPosts = async (currentUserId?: string): Promise<Post[]> => {
    ...
    const mockPosts: Post[] = [
      {
        ...
        created_at: new Date(Date.now() - 3600000).toISOString(), likes_count: 12, comments_count: 2, user_has_liked: false, user_has_saved: false,
        recent_liked_avatars: ['https://i.pravatar.cc/150?u=20', 'https://i.pravatar.cc/150?u=33', 'https://i.pravatar.cc/150?u=44'],
      ...
      },
      {
        ...
        created_at: new Date(Date.now() - 86400000).toISOString(), likes_count: 45, comments_count: 10, user_has_liked: true, user_has_saved: true,
        recent_liked_avatars: ['https://i.pravatar.cc/150?u=5', 'https://i.pravatar.cc/150?u=6', 'https://i.pravatar.cc/150?u=7'],
  ...
  // Busca posts e faz join com profiles, saved_posts, e likes
  const { data, error } = await supabase
    .from('posts')
    .select(`
      *,
      profiles:user_id ( id, username, avatar_url, role ),
      post_likes ( user_id, profiles:user_id(avatar_url) ),
      saved_posts ( user_id ),
      comments ( id )
    `)
    .order('created_at', { ascending: false });
  ...
  // Processa dados para formato do frontend
  return data.map((post: any) => {
      // Process avatars: Take first 3 unique valid avatars
      const avatars = post.post_likes
      ...
      return {
        ...
        user_has_liked: currentUserId ? post.post_likes.some((like: any) => like.user_id === currentUserId) : false,
        user_has_saved: currentUserId ? post.saved_posts.some((save: any) => save.user_id === currentUserId) : false,
        recent_liked_avatars: avatars,
