    ...
    <aside className="w-64 h-screen bg-surface border-r border-border flex flex-col fixed left-0 top-0 z-20 hidden md:flex">
      ...
      <nav className="flex-1 px-4 py-6 space-y-1">
        {NAVIGATION_ITEMS.map((item) => (
          <button
            key={item.id}
            onClick={() => onTabChange && onTabChange(item.id === 'contents' ? TabOption.CONTENTS : TabOption.HOME)}
            className={`
              w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
              ${(item.id === 'home' && activeTab === TabOption.HOME) || (item.id === 'contents' && activeTab === TabOption.CONTENTS)
